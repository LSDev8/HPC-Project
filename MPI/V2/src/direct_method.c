...
/* BUFFER Pj et Fj */
/*******************/
COORDINATES_T *pj_pos_x, *pj_pos_y, *pj_pos_z;
COORDINATES_T *pj_fx, *pj_fy, *pj_fz;
...
/****************************/
void Direct_method_Bodies_Init(bodies_ind_t nb_bodies)
{
  bodies_ind_t korps;
  ...
  for (korps = 0; korps < nb_bodies; ++korps)
    {
      pj_pos_x[korps] = 0;
      pj_pos_y[korps] = 0;
      pj_pos_z[korps] = 0;
...
/* Free Pj*/
/**********/
void Direct_method_Bodies_Terminate()
{
  FMB_free(pj_pos_x);
  FMB_free(pj_pos_y);
  FMB_free(pj_pos_z);
...
/***********************************/
/* SEND DATA to proc P from root 0 */
/***********************************/
void Direct_method_ISend(int Proc, position_t *spd, MPI_Request *req)
{
  MPI_Isend(pj_pos_x, bodies.nb_bodies, MPI_FLOAT, Proc, 1, MPI_COMM_WORLD, req);
  MPI_Isend(pj_pos_y, bodies.nb_bodies, MPI_FLOAT, Proc, 2, MPI_COMM_WORLD, req+1);
  MPI_Isend(pj_pos_z, bodies.nb_bodies, MPI_FLOAT, Proc, 3, MPI_COMM_WORLD, req+2);
  MPI_Isend(spd, bodies.nb_bodies * 3, MPI_FLOAT, Proc, 5, MPI_COMM_WORLD, req+3);
}

/*****************************/
/* Init Bobies and Recv data */
/*****************************/
void Direct_method_InitIRecv()
{
  ...
  bodies.p_values = p_allvalues + nb_bodies * (rank - 1);

  /* On place le pointeur des masses */
  p_values = bodies.p_values + nb_bodies;

  /* Reception des corps (position et vitesses) */
  MPI_Irecv(bodies.p_pos_x, nb_bodies, MPI_FLOAT, 0, 1, MPI_COMM_WORLD, req);
  MPI_Irecv(bodies.p_pos_y, nb_bodies, MPI_FLOAT, 0, 2, MPI_COMM_WORLD, req+1);
  MPI_Irecv(bodies.p_pos_z, nb_bodies, MPI_FLOAT, 0, 3, MPI_COMM_WORLD, req+2);
  MPI_Irecv(bodies.p_speed_vectors, nb_bodies * 3, MPI_FLOAT, 0, 5, MPI_COMM_WORLD, req+3);
  bodies_ClearFP(&bodies);

  /* Recuperation des masses */
  MPI_Bcast(p_allvalues, nb_bodies_total, MPI_FLOAT, 0, MPI_COMM_WORLD);

  /* Initialisation des buffers Pj */
  Direct_method_Bodies_Init(nb_bodies);
...
void Direct_method_Data(char *data_file){
  ...
  for (i = 1; i <= nb_proc; ++i)
      ...
      for (k=0; k<nb_bodies; ++k)
	{
	  ...
	  if (i != 1)
	    {
	    MPI_Wait(id,NULL); MPI_Wait(id+1,NULL); MPI_Wait(id+2,NULL); MPI_Wait(id+3,NULL);
	    }
      ...
      if (i != nb_proc)
	{
	  ...
	  Direct_method_ISend(i, spd, id);
	}
      current_values += nb_bodies;
    }
  MPI_Bcast(p_allvalues, nb_bodies * nb_proc, MPI_FLOAT, 0, MPI_COMM_WORLD);
  bodies_ClearFP(&bodies);
  FMB_free(spd);
...
void Direct_method_Sum(char *results_file,
  ...
  MPI_Reduce(&force_sum, &force_sum_total, 3, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
