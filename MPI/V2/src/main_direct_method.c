...
int main(int argc, char **argv){
  ...
  while ( tnow-FMB_Info.dt < tend )
      ...
      if(tnow!=0)
	{
      ...
      my_send(bodies.p_pos_x, bodies.p_pos_y, bodies.p_pos_z, POS(rank - 1) % nb_proc, 0, idps); //Pi
      my_recv(pj_pos_x, pj_pos_y, pj_pos_z, (rank+1) % nb_proc, 0, idpr); //Pj
      ...
      for (i = 1; i <= (nb_proc-1) / 2 ; ++i)
	{
	  ...
	  my_send(pj_pos_x, pj_pos_y, pj_pos_z, POS(rank-1) % nb_proc, 0, idps);
	  my_recv(pjrecv_pos_x, pjrecv_pos_y, pjrecv_pos_z, (rank+1) % nb_proc, 0, idpr);
	  ...
	  my_send(pjsend_fx, pjsend_fy, pjsend_fz, (rank+i) % nb_proc, 1, idfs); // Fj
	  my_recv(pjrecv_fx, pjrecv_fy, pjrecv_fz, POS(rank-i) % nb_proc, 1, idfr); // Fj
...
#undef TMP_STRING_LENGTH
  ...
  if (rank == 0)
	...
	bodies_Free(&bodies_all);
      ...
      Direct_method_Bodies_Terminate();
    }
  else
    {
